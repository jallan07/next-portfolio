    ...
    <Timeline>
      {recommendationItems.map((item, i) => (
        <Timeline.Item key={item?.name}>
          ...
          <Timeline.Content>
            {showDate && item?.date && (
              <Timeline.Time className="text-zinc-600 dark:text-zinc-500">
                {formatDate(item.date)}
              </Timeline.Time>
            )}
            <Timeline.Title>{item?.name}</Timeline.Title>
            <p className="text-sm font-normal italic text-zinc-600 dark:text-zinc-500">
              {item?.position}
            </p>
            <Timeline.Body className="text-zinc-600 dark:text-zinc-400">
              <p className="recommendation-body">{item?.description}</p>
            </Timeline.Body>
            {showLink && (
              <a href={item?.href} target="_blank" rel="noreferrer">
                <Button>
                  See {item?.name.split(' ')[0]}'s full profile on LinkedIn
                  <NewSiteIcon className="h-3 w-3" />
                </Button>
              </a>
            )}
          </Timeline.Content>
