      ...
      {resumeItems.map((item, i) => (
        <Timeline.Item key={item.title + i}>
          ...
          <Timeline.Content>
            ...
            >{`${item.dates.start} - ${item.dates.end}`}</Timeline.Time>
            ...
            <Timeline.Body className="text-zinc-600 dark:text-zinc-400">
              ...
              {item.keyAchievements && !homeButtons && (
                <div className="text-md my-3 ml-2">
                  <Timeline.Title className="text-md">
                    Key Achievements:
                  </Timeline.Title>
                  {item.keyAchievements.map((keyAchievement, i) => (
                    <ul className="ml-8 list-outside list-disc" key={i}>
